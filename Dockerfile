...
COPY --chown=www-data:www-data build /usr/local/apache2/htdocs/gridmonitor

# setup ssi for base in index.html using the per request variable defined in app-httpd.conf
RUN sed -i -e 's;<base href="/" />;<base href="<!--#echo var="BASE" -->" />;' /usr/local/apache2/htdocs/gridmonitor/index.html
